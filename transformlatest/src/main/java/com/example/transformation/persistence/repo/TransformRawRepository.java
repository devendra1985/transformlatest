...
public class TransformRawRepository {
  private final DbClient db;
  private final String table;

  public TransformRawRepository(DbClient db, String table) {
    this.db = db;
    this.table = table;
  }
  ...
  public void insert(PersistenceRecord r) {
    db.update(
        "insert into " + table + " (REQUEST_ID, RAW_PAYLOAD, STATUS, CREATED_AT) values (?, ?, ?, ?)",
        r.requestId(),
        r.payloadJson(),
        r.status(),
        r.createdAt()
